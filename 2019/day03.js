...
const createPath = (instruction, create = true) => {
    const direction = instruction.slice(0, 1)
    const length = instruction.slice(1)
    const axis = ['L', 'R'].includes(direction) ? 'x' : 'y'
    const step = ['R', 'U'].includes(direction) ? 1 : -1

    for (let l = 0; l < length; ++l) {
        pos.steps++
        pos[axis] = pos[axis] + step
        const str = `${pos.x},${pos.y}`
        if (create) {
            path.set(str, Math.min(pos.steps, path.get(str) || Number.MAX_SAFE_INTEGER))
        }
        else if (path.has(str)) {
            closest = Math.min(closest, Math.abs(pos.x) + Math.abs(pos.y))
            intersections.set(str, pos.steps + path.get(str))
        }
    }
}

const findIntersections = instruction => createPath(instruction, false)

const run = wires => {
    reset()
    wires.a.forEach(createPath);
    reset()
    wires.b.forEach(findIntersections);
}

run(puzzle)

const partOne = () => closest
const partTwo = () => Math.min(...[...intersections.values()])

console.log('Part One:', partOne()) // 557
console.log('Part Two:', partTwo()) // 56410
